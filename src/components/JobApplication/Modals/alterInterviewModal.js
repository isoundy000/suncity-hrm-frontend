...
function AlterInterviewModal({
  ...
  if (currentInterviewModalData != null) {
    ...
    console.log('//', interviewer_users);
    var users = [];
    if (interviewer_users != null) {
      users = interviewer_users.map(item => {
        return `${item.empoid}||${item.chinese_name}||${item.english_name}||-1||-1||${item.email}`;
      });
    }
  }
  ...
  function handleOk() {
    form.validateFields((errors) => {
      ...
      data.old_interviewer_emails = users;
      ...
      const trueData = {
        ...
        old_interviewer_emails: data.old_interviewer_emails.map(interviewer => interviewer.split('||')[5]),
      }
      ...
      if (data.need_email === true) {
        console.log('true', data.interviewer_emails);
  ...
  function handleSelectSearch(value) {
    ...
    timeOut = setTimeout(doIt, 666);
  }

  function usersToEmails(users) {
    console.log('ute', users);
    if (users) {
      ...
      setTimeout(() => {
        ...
        if (notExist) {
          ...
          const realValues = selectedEmails.map(email => {
            const tmp = searchResult.filter(user => user.email === email);
            if (tmp.length > 0) {
              return tmp[0];
            } else {
              return value;
          ...
          console.log('realValues', selectedEmails, realValues);
          dispatch({
    ...
    if (!value) {
      ...
      setTimeout(() => {
        ...
        for (const option of value) {
          const allEmails = [...emailList, ...validatedEmail, ...users];
          if (allEmails.findIndex(email => option === email) < 0) {
            notExist = true;
          }
        }
  ...
  const modalOpts = {
    ...
    // onOk: handleOk,
    title: "修改面試",
    visible: alterInterviewModalStatus,
    okText: okText,
    ...
    >
      <Form horizontal>
        ...
        <FormItem
          ...
          label="面試輪次"
        >
          <div style={{position: "relative"}} id="fixedSelect1">
          {getFieldDecorator('mark', {initialValue: mark, rules: [{required: true,message:notNull}]})(
             <Select id="select" size="large" style={{width: 200}}
                     getPopupContainer={() => document.getElementById('fixedSelect1')}>
        ...
        >
          <Col span="15">
            {getFieldDecorator('interviewer_emails', {
               initialValue: users != null ? users : null,
               rules: [
...
export default Form.create()(injectIntl(AlterInterviewModal));
