...
function RefuseInterviewModal({ myRecruit, dispatch, cardData, nth, ...props }) {
  const { formatMessage } = props.intl;

  const interviewRefusedText = formatMessage(messages['app.my_recruit.tag_card.interview_refused']);

  const refuseInterviewText = formatMessage(messages['app.my_recruit.card_modal.refuse_interview']);
  ...
  const modalType = 'refuseInterviewModal';
  ...
  const handleOk = () => {
    ...
    dispatch({
      ...
      payload: {
        status: `${nth}_interview_rejected`,
  ...
  return (
    <div className={styles.refuseButton} >
      <Button onClick={handleOnClick}>{refuseInterviewText}</Button>
      <Modal
        title={interviewRefusedText}
...
RefuseInterviewModal.propTypes = {
  cardData: React.PropTypes.object.isRequired,
};

RefuseInterviewModal = createForm()(RefuseInterviewModal);

const mapStateToProps = ({ myRecruit }) => ({
  myRecruit,
});

export default connect(mapStateToProps)(injectIntl(RefuseInterviewModal));
